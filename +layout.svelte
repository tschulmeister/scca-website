...
<div class="min-h-screen bg-slate-50 flex flex-col" style="--page-header-bg: url({communitySign});">
  <!-- Navigation Header -->
  <header class="relative z-30 bg-slate-900/95 text-white shadow-lg shadow-slate-900/10 backdrop-blur-sm border-b border-slate-800">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="flex flex-col gap-4 py-4 md:flex-row md:items-center md:justify-between">
        <div class="flex items-center justify-between gap-4">
          <div>
            <span class="text-xl font-bold tracking-tight">Shipley's Choice</span>
        ...
        <nav bind:this={navWrapper} class="hidden flex-wrap items-center gap-2 md:flex md:justify-end" aria-label="Primary navigation">
          {#each visibleLinks as link}
            <a
              href={link.path}
              class="inline-flex items-center px-3 py-2 text-sm transition-colors duration-150 { $page.url.pathname === link.path ? 'border-b-2 border-blue-400 text-white font-semibold' : 'border-b-2 border-transparent text-slate-200 hover:text-white hover:border-slate-500' }"
            >
              {link.name}
            </a>
          {/each}
          ...
          {#if overflowLinks.length}
            <div class="relative">
              ...
              {#if moreOpen}
                <div class="absolute right-0 z-50 mt-2 w-48 overflow-hidden rounded-2xl border border-slate-700 bg-slate-900/95 shadow-lg shadow-slate-900/30 backdrop-blur-sm">
  ...
  <div bind:this={measureContainer} class="pointer-events-none absolute left-[-9999px] top-[-9999px] opacity-0" aria-hidden="true">
    {#each navLinks as link}
      <span use:trackMeasure={link} class="inline-flex items-center px-3 py-2 text-sm font-medium">
        {link.name}
      </span>
    {/each}
    <span bind:this={moreButton} class="inline-flex items-center px-3 py-2 text-sm font-medium">
      More..
    </span>
  </div>
  ...
  <footer class="bg-white border-t border-slate-200 p-8 text-center text-slate-500 text-sm">
    &copy; {new Date().getFullYear()} Shipley's Choice Community Association. All rights reserved.
  </footer>
</div>