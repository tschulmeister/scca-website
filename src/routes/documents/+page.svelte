<script>
    ...
    const documents = [
        ...
        {
            filename: "scca_articles_of_incorporation.pdf",
    ...
    >
        <div>
            ...
            <p class="text-sm text-slate-500">
                Switch between documents and neighorhood plats images by using
                the toggle switch
            </p>
        </div>
    ...
    >
        ...
        >
            <div class="p-4 border-b border-slate-100 bg-slate-50/50">
                ...
                >
                    Available {activeTab === "docs"
                        ? "Documents"
                        : "Plat Records"} ({activeTab === "docs"
                        ? documents.length
                        : plats.length})
                </h2>
            </div>
                ...
                {#each activeTab === "docs" ? documents : plats as file}
                    ...
                    >
                        {#if file.filename.endsWith(".pdf")}
                            <svg
                                class="h-5 w-5 shrink-0 {selectedFile?.path ===
                                file.path
                                    ? 'text-blue-600'
                    ...
                    {:else if fileExtension === "docx"}
                        ...
                        >
                            ...
                            <p class="text-sm text-slate-500 mb-5">
                                Microsoft Word application files cannot be
                                directly embedded inside the web browser frame.
                                Use the button below to download the local
                                resource form file.
                            </p>
                            <a
                                href={selectedFile.path}
            ...
            {:else}
                <div class="text-center p-8 select-none">
                    ...
                    <p class="text-sm font-medium text-slate-500">
                        Select a file from the sidebar list to display preview
                        information.
                    </p>
                </div>
            {/if}
